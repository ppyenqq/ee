...
int main() {
	...
	automat->step('?');
	cout <<"not allowed sign ?  "<< automat->getTokenType() << endl;
	automat->reset();
	cout <<"bool step(a)  "<< automat->step('a') << endl;
	automat->reset();
	cout <<"bool step(=)  "<< automat->step('=') << endl;
	automat->reset();
	cout <<"bool step(3)  "<< automat->step('3') << endl;
	automat->reset();
	cout <<"bool step(?)  "<< automat->step('?') << endl;
	automat->reset();
	cout <<"bool step( )  "<< automat->step(' ') << endl;
	...
	automat->step('1');
	automat->step('b');
	cout <<"TType bevor Error  "<< automat->getTokenType() << endl;
	automat->step('?');
	cout <<"TType nach Error  "<< automat->getTokenType() << endl;
	automat->reset();
